...
import ScientificComputingInLean.Meta.Lean.Scopes
...
def lean : CodeBlockExpander
  | args, str => do
    ...
    repeat
      ...
      cmdState ← withInfoTreeContext (mkInfoTree := pure ∘ InfoTree.node (.ofCommandInfo {elaborator := `ScientificComputingInLean.Meta.lean, stx := cmd})) do
...
partial def name : RoleExpander
  | args, #[arg] => do
    ...
    let _resolvedName ← withInfoTreeContext (mkInfoTree := pure ∘ InfoTree.node (.ofCommandInfo {elaborator := `ScientificComputingInLean.Meta.name, stx := identStx})) <| realizeGlobalConstNoOverloadWithInfo identStx
