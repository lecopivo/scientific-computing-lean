...
by
  ...
  -- apply RK4 method
  simp_rw -zeta [odeSolve_fixed_dt rungeKutta4 sorry_proof]

  -- approximate the limit with a fixed `n`
  approx_limit n sorry_proof
```
There are two main steps in turning the specification into implementation. We need to compute the derivatives `∇ (p':=p), H m k x  p'` and `∇ (x':=x), H m k x' p` and we need to approximate the function {name}`odeSolve` using some time stepping scheme.

To compute the derivatives, we unfold the definition of the Hamiltonian {lean}`H` and call `autodiff` tactic.

To approximate {name}`odeSolve` we rewrite the specification using the theorem {name}`odeSolve_fixed_dt` saying that {name}`odeSolve` can be approximated by a time stepping scheme, we choose Runge-Kutta 4 {name}`rungeKutta4`.
j
The theorem {name}`odeSolve_fixed_dt` produces a specification of the form `limit n → ∞, ...`. We can decide to approximate this limit by taking a concrete value `n` and hope it is sufficiently large such that the approximation is good enough. To do this we call the tactic `approx_limit` and we end up with a goal that fully computable expression.
